...
include("helpers.jl")
gr()
function circles_in_box_linear_spring_hysteresis(L, H, T, dt, save_gif, verbose, initial_condition...)
	...
	for iteration_number=1:number_iterations
		...
		for pair in PCS
			...
			if (overlap>=0)
				...
				# Coulomb limit
				#if norm(pseudo_Ft)>epsilon
					#pseudo_Ft = min(norm(pseudo_Ft), mu_s*Fn)*pseudo_Ft/norm(pseudo_Ft)
				#end
	...
	if save_gif
		...
		gif(anim, "hysteresis24fps.gif", fps = 24)
	end
end
